...
class ThreadMongoSingleton:
    ...
    We use threading local singleton to make sure that the same instance is used in the same thread.
     Otherwise, we will get an error "RuntimeError: Event loop is closed" or
     "Future attached to a different loop". To save backward compatibility, we provide access
     to database with __call__ method which return a new thread local singleton instance.
    For example:
